...
function Uploader() {
  const { apiKey, navigate, setPendingBill, setPendingImage } = React.useContext(BS.AppContext);
  ...
  async function handleExtract() {
    if (!preview) return;
    if (!apiKey) { setErrorMsg('No API key set. Please go to Settings.'); return; }
    ...
    try {
      ...
      const result = await BS.claudeApi.extractBill(base64, mimeType, apiKey);
      ...
      if (err.message === 'NO_API_KEY') setErrorMsg('No API key set. Go to Settings.');
      else if (err.message === 'AUTH_ERROR') setErrorMsg('Invalid API key. Check Settings.');
    ...
    <div style={{ maxWidth: 680, margin: '0 auto' }}>
      ...
      {!apiKey && (
        <div className="alert alert-warn" style={{ marginBottom: 16 }}>
          ⚠ No API key configured. <button className="btn btn-ghost btn-sm" style={{ marginLeft: 8 }} onClick={() => navigate('settings')}>Go to Settings →</button>
        </div>
      )}
      {!preview && (
        <div
