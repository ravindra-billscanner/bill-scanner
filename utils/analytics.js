// analytics.js — pure analytics functions, exposed on BS.analytics
(function() {
  function getAllItems(bills) {
    return bills.reduce((acc, b) => acc.concat((b.items || []).map(i => ({ ...i, billDate: b.date, billTotal: b.total, currency: b.currency }))), []);
  }
  ...
  function getCategoryBreakdown(bills) {
    ...
    const map = {};
    items.forEach(i => { const c = i.category || 'Other'; if (!map[c]) map[c] = { total: 0, count: 0 }; map[c].total += i.price || 0; map[c].count++; });
    const grand = Object.values(map).reduce((s, v) => s + v.total, 0);
    return Object.entries(map).map(([category, { total, count }]) => ({ category, total: +total.toFixed(2), count, percentage: grand > 0 ? +((total / grand) * 100).toFixed(1) : 0 })).sort((a, b) => b.total - a.total);
  }

  function getBrandFrequency(bills, topN) {
    topN = topN || 10;
    const map = {};
    getAllItems(bills).forEach(i => { if (!i.brand) return; const b = i.brand.trim(); if (!b) return; if (!map[b]) map[b] = { count: 0, totalSpent: 0 }; map[b].count++; map[b].totalSpent += i.price || 0; });
    return Object.entries(map).map(([brand, { count, totalSpent }]) => ({ brand, count, totalSpent: +totalSpent.toFixed(2) })).sort((a, b) => b.count - a.count).slice(0, topN);
  }

  function getStoreFrequency(bills, topN) {
    topN = topN || 10;
    const map = {};
    bills.forEach(b => { const s = (b.store_name || 'Unknown').trim(); if (!map[s]) map[s] = { visits: 0, totalSpent: 0 }; map[s].visits++; map[s].totalSpent += b.total || 0; });
    return Object.entries(map).map(([store, { visits, totalSpent }]) => ({ store, visits, totalSpent: +totalSpent.toFixed(2) })).sort((a, b) => b.visits - a.visits).slice(0, topN);
  }
  ...
  function getKpiSummary(bills) {
    ...
    return { totalBills: bills.length, totalSpend, avgBillValue: +(totalSpend / bills.length).toFixed(2), topStore: (stores[0] || {}).store || '—', topBrand: (brands[0] || {}).brand || '—', topCategory: (cats[0] || {}).category || '—' };
  }
  ...
  function computeSegment(bills) {
    ...
    let bpm = 0;
    if (dates.length >= 2) { dates.sort((a, b) => a - b); const months = (dates[dates.length - 1] - dates[0]) / (1000 * 60 * 60 * 24 * 30.5); bpm = months > 0 ? bills.length / months : bills.length; }
    const items = getAllItems(bills);
  ...
  BS.analytics = { getSpendingByMonth, getCategoryBreakdown, getBrandFrequency, getStoreFrequency, getKpiSummary, computeSegment };
})();
